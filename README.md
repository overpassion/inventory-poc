...
DB는 **WAL 모드**(변경내용을 먼저 로그에 적는 방식)로 돈다. 그래서 서버가 켜져 있는 동안은 `dev.db-wal` · `dev.db-shm` 이 함께 있다.

**이 파일들이 보이면 서버가 깨끗이 멈추지 않은 것이다.** 아직 본 파일에 반영되지 않은 변경이 `-wal` 에 남아 있으므로, `dev.db` 만 복사하면 **최근 기록을 잃는다.** 셋을 모두 복사하거나, 서버를 제대로 멈춰 파일이 사라진 뒤에 복사한다.
