# 06. 아키텍처 — 대부분 `harness/ssot.md` 로 이전됨

> 원래 작성: Phase 6 · 이전: 2026-09-15
>
> **규범 사양은 [`harness/ssot.md`](./harness/ssot.md) 에 있다.**
> 중복을 없애기 위해 본문을 옮겼고, **`ssot.md` 가 담지 않은 두 절만 여기 남겼다** — §3 데이터 모델(필드 수준)과 §7 시드 데이터 설계.

## 어디로 갔나

| 원래 절 | 옮겨간 곳 |
|---|---|
| 0. 이 앱이 실행되는 방식 | `ssot.md` §3.1 |
| 1. 스택 확정 · 실제 설치 버전 | `ssot.md` `ARCH-01`~`ARCH-08` (§3.1) |
| 1. 채택하지 않은 것 | `ssot.md` `ADR-01`~`ADR-06` (§3.2) |
| 2. 폴더 구조 | `ssot.md` §3.4 |
| 4.1 `applyMovement()` | `ssot.md` §3.5 · `RULE-01`~`RULE-03` `RULE-09` |
| 4.2 로트 배분 FEFO/LEFO | `ssot.md` §3.5 · `RULE-04`~`RULE-06` |
| 4.3 팝업 정산 역산 | `ssot.md` §3.5 · `RULE-07` |
| 4.4 취소 = 상쇄 기록 | `ssot.md` §3.5 · `RULE-08` |
| 5. 상태 흐름 | `ssot.md` `DOM-08` |
| 6. 인증 | `ssot.md` §3.6 |
| 7.5 아키텍처 결정 요약 | `ssot.md` §3.3 |
| 8. 동시성 · 성능 | `ssot.md` §3.7 |
| 9. 자동 테스트로 지킬 불변식 | `ssot.md` `INV-01`~`INV-07` (§4.1) |

## 여기 남은 것

| 절 | 왜 남겼나 |
|---|---|
| **§3 데이터 모델** | `ssot.md` `DOM-04` 는 엔터티 **이름 9개**만 담는다. 필드 · 인덱스 · 관계는 이 문서가 유일하다 |
| **§7 시드 데이터 설계** | `ssot.md` 에 없다. 시연이 끊기지 않게 하는 시드 구성은 규범에 가깝지만 이번 추출 대상이 아니었다 |

> **아래 두 절에는 알려진 어긋남이 있다. 임의로 고치지 않고 남겨둔다** (`ssot.md` §0.5 — 코드는 판단 기준이 아니다).
>
...
`ssot.md` §0.6 기준 **아키텍처 영역** — AI 는 읽기만 하고, 변경·승인은 사람이 한다.
