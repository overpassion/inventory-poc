...
- 화면 캡처는 `docs/screenshots/` (일일 반영은 16~21번)
- **팝업(M6)은 아직 브라우저 클릭 확인 전이다.** 도메인 로직은 `popup-settle.test.ts`가,
  화면은 서버 렌더 HTML이 검증됐다. 시드의 성수 팝업으로 한 번 눌러 볼 것 (아래 2절)
...
### ① 팝업 클릭 확인 (10분)
시드의 **성수 팝업(누적 반출 160 = 120 + 40)** 이 정산 대기 상태다.
`/popups` → 성수 팝업 → `정산 시작` → 로트별 잔여 합계 42 · 시식 5 입력 →
문장이 **"113개를 판매로 기록합니다"** 로 뜨면 확정. 확정 후 팝업 거점 0 · 자사창고 +42 · 리포트 표시.
되돌리기 버튼으로 원상 복구되는 것까지 확인하면 M6 검증 끝.
